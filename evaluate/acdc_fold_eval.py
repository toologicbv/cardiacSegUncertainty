...
from evaluate.fold_eval import FoldEvaluation
...
class ACDCFoldEvaluation(FoldEvaluation):

    def __init__(self, model_name, fold, result_root_dir='~/models/dummy', type_of_map="e_map"):

        super(FoldEvaluation, self).__init__(model_name, fold, result_root_dir, type_of_map=type_of_map)
    ...
    def save(self, file_name=None):
        ...
        if file_name is None:
            file_name = "{}_f{}_".format(self.dice.shape[0], self.fold_id) + FoldEvaluation.file_extension
    ...
    @staticmethod
    def patient_id_to_int(patient_id):
        return int(patient_id.strip("patient"))

    @staticmethod
    def int_to_patient_id(p_id):
        return "patient" + str(p_id).zfill(3)
